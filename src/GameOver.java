...
public class GameOver {
    ...
    public GameOver(){
        ...
        Miss1 =new JLabel(String.valueOf(MineClearance.SB.getMistake()));
        Miss2 =new JLabel(String.valueOf(MineClearance.PS.getMistake()));
        Point1 =new JLabel(String.valueOf(MineClearance.SB.getScore()));
        Point2 =new JLabel(String.valueOf(MineClearance.PS.getScore()));
        Title =new JLabel("Score       Miss");
        Player1=new JLabel(MineClearance.SB.getName1());
        Player2=new JLabel(MineClearance.PS.getName2());

        if (MineClearance.SB.getScore()>MineClearance.PS.getScore()){
            Winner =new JLabel("胜利者是"+MineClearance.SB.getName1()+" !");
        }
        else if (MineClearance.SB.getScore()<MineClearance.PS.getScore()){
            Winner =new JLabel("胜利者是"+MineClearance.PS.getName2()+" ！");
        }

        else {
            if (MineClearance.SB.getMistake()<MineClearance.PS.getMistake()){
                Winner =new JLabel("胜利者是"+MineClearance.SB.getName1()+" !!!!!!");
            }
            else if(MineClearance.SB.getMistake()>MineClearance.PS.getMistake()){
                Winner =new JLabel("胜利者是"+MineClearance.PS.getName2()+" !!!!!!");
            }
            else {
